...
function getColors (id)
{
    ...
    const result = {
        green : window.getComputedStyle(tmp.querySelector('.bar-on-time')).backgroundColor,
        red   : window.getComputedStyle(tmp.querySelector('.bar-late')).backgroundColor,
    };
...
export function latePaymentHistogramChart (id, data, target = null)
{
    ...
    const chartData = {
        ...
        datasets : [
            {
                data,
                backgroundColor    : ({dataIndex}) => !dataIndex ? color.green : color.red,
                barPercentage      : 1.0,
